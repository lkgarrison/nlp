...
class Tree(object):
	...
	def vertical_markovization(self):
		self.vertical_markovization_helper(self.root, '')

	def vertical_markovization_helper(self, node, parent_label):
		if len(node.children) == 0:
			return

		for child in node.children:
			self.vertical_markovization_helper(child, '^' + node.label)

		node.label += parent_label

	def undo_vertical_markovization(self):
		self.undo_vertical_markovization_helper(self.root)

	def undo_vertical_markovization_helper(self, node):
		if '^' in node.label:
			node.label = node.label.split('^')[0]

		for child in node.children:
			self.undo_vertical_markovization_helper(child)
